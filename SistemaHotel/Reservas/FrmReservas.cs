...
namespace SistemaHotel.Reservas
{
    public partial class FrmReservas : Form
    {
        ...
        {
            quantidadesPessoas = txtPessoas.Text;


            //CÓDIGO DO BOTÃO PARA SALVAR
            con.AbrirCon();
            sql = "INSERT INTO ocupacoes (quarto, data, funcionario, pessoas) VALUES (@quarto, @data, @funcionario, @pessoas)";
            cmd = new MySqlCommand(sql, con.con);
            cmd.Parameters.AddWithValue("@quarto", cbQuarto.Text);
            cmd.Parameters.AddWithValue("@data", Convert.ToDateTime(dataPainel));
            cmd.Parameters.AddWithValue("@funcionario", Program.nomeUsuario);
            cmd.Parameters.AddWithValue("@pessoas", quantidadesPessoas);
        ...
        private void AtualizarTotalReserva()
        {
            try { 
            diasReserva += 1;
            quantidadePessoas = Convert.ToDouble(txtPessoas.Text);
            txtDias.Text = diasReserva.ToString();
            valorTotal = diasReserva * quantidadePessoas * Convert.ToDouble(txtValor.Text);
            lblTotal.Text = string.Format("{0:c2}", valorTotal);
            }
            catch(Exception)
            {
                MessageBox.Show("Selecione a quantidade de pessoas para o quarto");
            }
        }
        ...
        private void AbaterTotalReserva()
        {
            try {
                ...
                lblTotal.Text = string.Format("{0:c2}", valorTotal * Convert.ToDouble(quantidadePessoas));

                }catch(Exception)
            {
                MessageBox.Show("Erro ao excluir reserva inexistente");
            }
        }
        ...
        private void BtnSalvar_Click(object sender, EventArgs e)
        {
            ...
            if (valorTotal > 0)
            {
                ...
                if (resultado == DialogResult.Yes)
                {
                    //CÓDIGO DO BOTÃO PARA SALVAR
                    con.AbrirCon();
                    sql = "INSERT INTO reservas (quarto, entrada, saida, dias, valor, nome, telefone, data, funcionario, pessoas) VALUES (@quarto, @entrada, @saida, @dias, @valor, @nome, @telefone, curDate(), @funcionario, @pessoas)";
                    cmd = new MySqlCommand(sql, con.con);
                    cmd.Parameters.AddWithValue("@quarto", cbQuarto.Text);
